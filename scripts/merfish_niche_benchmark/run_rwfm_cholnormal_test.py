"""Diagnostic: run RiemannianWassersteinFlowMatching (geom='euclidean') with the new
'chol_normal' noise type (ported from the original wasserstein module -- see
utils_Noise.chol_normal_noise) instead of the default 'ambient_gaussian', on the exact same
Sst/k=16/16-PC replication used to test the original code. If this closes the gap to the paper's
reported ~53-56% 1-NNA, it pins the discrepancy on the noise source (specifically the
noise_df_scale point-widening the riemannian_wasserstein port dropped) rather than on any other
difference between the two library code paths (architecture, training loop, generation
integrator, etc).

Example:
    python run_rwfm_cholnormal_test.py --noise-type chol_normal
    python run_rwfm_cholnormal_test.py --noise-type ambient_gaussian   # control run
"""
...
def _parse_args():
    ...
    p.add_argument('--noise-type', type=str, default='chol_normal',
                   choices=['chol_normal', 'ambient_gaussian'])
    ...
    p.add_argument('--num-sinkhorn-iters', type=str, default='converge',
                   help="main-map Sinkhorn iterations: 'converge' (None), '-1' (auto-find), or an int")
    p.add_argument('--mini-batch-ot-num-iter', type=str, default='converge',
                   help="mini-batch OT Sinkhorn iterations: 'converge' (None), '-1' (auto-find), or an int")
...
def main():
    ...
    print(f"[rwfm-test] noise_type={args.noise_type} monge_map={args.monge_map} "
    ...
    print(f"[rwfm-test] {len(train_clouds)} train / {n_real} test niches "
    ...
    print(f"[rwfm-test] real-real blocks done ({time.time() - t0:.0f}s)", flush=True)

    model = RiemannianWassersteinFlowMatching(
        point_clouds=train_clouds, geom='euclidean', monge_map=args.monge_map,
        mini_batch_ot_mode=True, num_sinkhorn_iters=_iters(args.num_sinkhorn_iters),
        mini_batch_ot_num_iter=_iters(args.mini_batch_ot_num_iter),
    ...
    print(f"[rwfm-test] trained {args.n_train_steps} steps in {train_wall:.0f}s "
          f"({time.time() - t0:.0f}s)", flush=True)
    ...
    for g in range(args.n_gens):
        ...
        while got < n_real:
            ...
            samples = model.generate_samples(num_samples=n, size=args.k,
                                             timesteps=args.timesteps, key=subkey)[0]
        ...
        gens.append(res)
        print(f"[rwfm-test] gen {g + 1}/{args.n_gens}: "
    ...
    print(f"[rwfm-test] FINAL noise_type={args.noise_type} monge_map={args.monge_map}: "
