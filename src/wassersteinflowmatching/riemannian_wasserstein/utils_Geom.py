...
class euclidean:
    ...
    def distance_matrix(self, P0, P1):
        sq0 = jnp.sum(P0**2, axis=-1)
        sq1 = jnp.sum(P1**2, axis=-1)
        return jnp.maximum(sq0[:, None] + sq1[None, :] - 2 * (P0 @ P1.T), 0.0)
...
class generic_riemannian:
    ...
    def __init__(self, n_interpolation_steps=10):
    ...
    def velocity(self, P0, P1, t):
        """
        Velocity of the geodesic from P0 to P1 at time t.
        For constant-speed geodesics this equals the log map (independent of t).
        """
        return jnp.nan_to_num(self._log_map(P0, P1), nan=0.0)
    ...
    def exponential_map(self, p, v, delta_t):
        ...
        Reduces to the first-order retraction when n_interpolation_steps=1.
        """
        N = jnp.maximum(1, jnp.round(self.n_interpolation_steps * delta_t)).astype(int)
        sub_dt = delta_t / N
        ...
        (result, _), _ = jax.lax.scan(body, (p, v), jnp.arange(self.n_interpolation_steps))
...
class mystery_sphere(generic_riemannian):
    ...
    def __init__(self, n_interpolation_steps=100):
        super().__init__(n_interpolation_steps=n_interpolation_steps)
